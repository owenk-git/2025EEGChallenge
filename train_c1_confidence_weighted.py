#!/usr/bin/env python3
"""
C1 Training with Confidence-Weighted Loss

Give more weight to extreme RTs (very fast/slow)
to prevent model from only learning mean values
"""
...
class ConfidenceWeightedLoss(nn.Module):
    """
    Weight loss by distance from mean
    Extreme values (fast/slow RTs) get higher weight
    """
    def __init__(self, weight_strength=1.0):
        super().__init__()
        self.weight_strength = weight_strength

    def forward(self, predictions, targets):
        # Compute sample weights based on distance from mean
        ...
        return weighted_loss, {
            'unweighted_mse': squared_errors.mean().item(),
            'weighted_mse': weighted_loss.item(),
...
def train_epoch(model, train_loader, criterion, optimizer, device):
    ...
    for X_batch, y_batch in pbar:
        ...
        pbar.set_postfix({
            'loss': f"{loss.item():.4f}",
            'mean_weight': f"{loss_dict['mean_weight']:.3f}"
        })
...
def main():
    parser = argparse.ArgumentParser(description='Train C1 with Confidence-Weighted Loss')
    parser.add_argument('--epochs', type=int, default=100)
    parser.add_argument('--batch_size', type=int, default=32)
    parser.add_argument('--lr', type=float, default=1e-3)
    parser.add_argument('--weight_strength', type=float, default=1.0,
                       help='Weight strength for extreme values (0 = no weighting, 1.0 = moderate, 2.0 = strong)')
    ...
    criterion = ConfidenceWeightedLoss(weight_strength=args.weight_strength)
    ...
    print(f"\n{'='*60}")
    print(f"Training C1 with Confidence-Weighted Loss")
    print(f"Weight Strength: {args.weight_strength}")
    print(f"{'='*60}\n")
    ...
    for epoch in range(args.epochs):
        ...
        if val_nrmse < best_val_nrmse:
            ...
            checkpoint_path = checkpoint_dir / 'c1_confidence_weighted_best.pt'
            torch.save({
                'epoch': epoch,
