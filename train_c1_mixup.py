#!/usr/bin/env python3
"""
C1 Training with Mixup Augmentation

Mixup forces the model to predict intermediate values,
preventing prediction collapse
"""
...
def mixup_data(x, y, alpha=0.4):
    ...
    Args:
        ...
        alpha: Mixup parameter (controls mixing strength)
...
def train_epoch(model, train_loader, criterion, optimizer, device, mixup_alpha=0.4):
    model.train()
    total_loss = 0
    all_preds = []
    all_targets = []
...
def main():
    parser = argparse.ArgumentParser(description='Train C1 with Mixup')
    parser.add_argument('--epochs', type=int, default=100)
    parser.add_argument('--batch_size', type=int, default=32)
    parser.add_argument('--lr', type=float, default=1e-3)
    parser.add_argument('--mixup_alpha', type=float, default=0.4,
                       help='Mixup alpha parameter (0 = no mixup, 0.4 = moderate, 1.0 = strong)')
    ...
    print(f"\n{'='*60}")
    print(f"Training C1 with Mixup Augmentation")
    ...
    for epoch in range(args.epochs):
        ...
        if val_nrmse < best_val_nrmse:
            ...
            checkpoint_path = checkpoint_dir / 'c1_mixup_best.pt'
            torch.save({
                'epoch': epoch,
